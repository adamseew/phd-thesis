...
function [q,y] = evolve_model_euler(model,dat,init)
    ...
    u0 = init.u0;
    u1 = model.u(init.c_chain(1));

    i = 2;
    
    for init.t0+dat.delta_T:dat.delta_T:init.tf
        q0 = Ad*q0+model.B*model.est_u(u1,u0);
        u0 = u1;
        
        if i <= size(init.c_chain,2)
            eu1 = model.u(init.c_chain(i));
            i = i+1;
        end
...
\subsection{\color{red}Estimation of the period}
\label{sec:imp_period_est}

\subsection{\color{red}Estimation of the state}
