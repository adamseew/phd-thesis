...
In this section, we provide an algorithm to solve the \fref{pb:cov-pb}{Problem}. Let us recall briefly our objective of providing a set of paths (a tour) in the plan from \fref{def:plan}{Definition} to cover each point in a given space. We summarize such space with a set of vertices $v:=\{v_1,v_2,\dots\}$ that form a polygon. The robot is free to move within the polygon excluding polyugonal obstacles. These are described by another set of vertices per each obstacle $o_1:=\{o_{1,1},o_{1,2},\dots\},o_2:=\{o_{2,1},o_{2,2},\dots\},\dots$. There are a number of different approaches in the literature to solve this problem. We have summarized these approaches in detail in \fref{sec:soa-cov-path-plan}{Sections}\fref{sec:opti-cov}{--\hspace{-.8ex}} for mobile robots and in \fref{sec:cov-plan-aero}{Sections}\fref{sec:opti-aero-cov}{--\hspace{-.8ex}} for aerial robots specifically. In summary, the sub-class of motion planning that find the coverage tour of a given space is coverage path planning (CPP)~\citep{choset1998coverage}. The algorithms that find such tour are NP-hard~\citep{arkin2000approximation} and use either implicitly or explicitly the cellular decomposition in which the robot free space is divided into sub-regions that can be easily covered~\citep{choset2001coverage,galceran2013survey}.

There are numerous approach for cellular decomposition itself. Some use the decomposition into equally sized sub-regions that form a grid, and then visits only the sub-regions where the robot is free to move~\citep{galceran2013survey}. Another way is to sweep the polygon and divide it in sub-regions\findex{sub-regions} when the sweep line\findex{sweep line} encounters a change in connectiviy. We implement this latter class in \fref{sec:cell-deco}{Section}.
Once the solution algorithms divides the free space into sub-region, it builds a graph with vertices the sub-region and edges connecting the adjacent sub-regions\citep{choset2005principles}. A covering tour between the sub-region is then an exhaustive tour through the adjacency graph with, e.g., depth-first search\findex{depth-first search} algorithm\citep{choset2005principles}. This latter techniques result in the order of the coverage. However, there are multiple ways for generating the coverage motion inside the sub-region. We recall a common way is the boustrophedon motion, whereas a generic fixed-wing craft, such as the Opterra that we use for the precision agriculture scenario in \fref{sec:motivation}{Section}, has limited maneuverability~\citep{mannadiar2010optimal,xu2011optimal,xu2014efficient}. For a generic aerial robot tt is then preferred to have large turning radius~\citep{wang2017curvature}; to this end we propose Zamboni-like motion instead. We briefly introduced this motion in \fref{sec:path-wise}{Section}.


\subsection{Cellular decomposition}
\label{sec:cell-deco}\findex{cellular decomposition}

To define the cellular decomposition algorithms that decomposes the space to be covered into non-overlapping sub-regions called cells\findex{cells}, we define robot's free space\findex{free space} as $\mathcal{Q}^v$ for an inertial navigation frame $\mathcal{O}_W$. $\mathcal{Q}^v$ is then the space where the robot is free to move without intersecting an obstacle~\citep{choset2005principles}. $\mathcal{Q}^v\subseteq\mathbb{R}^2$ contains all the points delimited by the vertices of the polygon $v$ except for $i$ obstacles delimited by the vertices of $i$ polygons $o_i$. Let further $\mathcal{Q}^{o_i}\subset\mathbb{R}^2$ be the space delimited by the obstacle $o_i$. The entire space in the polygon $v$ including all the obstacles $o_i$ is then $\mathcal{Q}:=(\bigcup_{i\in|o|}\mathcal{Q}^{o_i})\cup\mathcal{Q}^v$.

An important approach in the polygonal environment is the boustrophedon decomposition\findex{boustrophedon decomposition}~\citep{choset2000coverage}. For non-polygonal environments where $v$ and $o_i$ are, e.g.,  elliptical functions, an important result is the decomposition in terms of critical points of Morse functions\findex{Morse functions}~\citep{choset2000exact}. Both the boustrophedon decomposition and decomposition in terms of critical points of Morse sweep $\mathcal{Q}$ and decompose $\mathcal{Q}^v$ adding a cell in case of a change of connectivity or when they encounter a critical point~\citep{choset2000coverage,choset2001coverage,choset2005principles}. This approach optimizes the neighboring cells that can be thus aggregated opposed to trapezoidal decomposition~\citep{galceran2013survey}--another method that simply adds a cell when it encounters a vertex~\citep{lavalle2006planning}. For the decomposition in terms of critical points of Morse functions, the intuition of using critical points\findex{critical points}~\citep{choset2000exact} has been studied in the planning literature also in the context of roadmaps~\citep{canny1988complexity,canny1988constructing,canny1993opportunistic}\findex{roadmaps}. We briefly summarize the main findings of Choset~et~al. for this latter method~\citep{choset2000exact} before providing the boustrophedon decomposition algorithm applied to out.

Let $\mathcal{Q}$ be the entire space delimited by vertices $v$ and $\mathcal{S}_\lambda$ the vertical slice that sweeps $\mathcal{Q}$. A change in the value of $\lambda$ moves the slice in $\mathcal{Q}$. Let further $\overline{x}_v,\underline{x}_v$ be the highest and lowest coordinate $x$ of all the vertices in $v$, i.e., $\lambda\in[\underline{x}_v,\overline{x}_v]$. We can express the entire space as the union of all the slice, i.e., $\mathcal{Q}=\cup_{\lambda}\mathcal{S}_\lambda$. Let us further define the slice contained in the free space $\mathcal{S}^v_\lambda:=\mathcal{S}_\lambda\cap\mathcal{Q}^{v}$.
At this point, a change in connectivity of $\mathcal{S}^v$ means that a new cell is needed, or that a cell is no longer necessary respectively when the connectivity increases or decreases~\citep{choset2000exact}. There are some other important findings derived from the planning literature of roadmaps~\citep{canny1988complexity,canny1988constructing,canny1993opportunistic} proving that connectivity changes happen at critical points of the boundary of the obstacle. Notably, the topology\findex{topology} (i.e., connectivity) changes only at critical points of the slice function restricted to the boundaries of obstacles.

For polygonal environments we know the vertices and the polygon, it is enough to verify the change in connectivity by iterating the vertices, and visit the constructed adjacency graph to find the coverage order. 

\begin{algorithm}[h!] %this is an example
  \SetKwInOut{Input}{Input}
  \SetKwInOut{Output}{Output}
  \vspace{.8ex}
  \Input{\otherfont 128 bit secret key}
  \Output{\otherfont 11 round keys each of 4 words as $w[0], \dots, w[43]$}
