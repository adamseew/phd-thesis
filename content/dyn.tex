...
\begin{algorithm}[h!] %this is an example
  ...
  \While{\normalsize $\mathbf{p}(i)\neq\mathbf{p}_{\Gamma_l}$}{
    \If{\normalsize $\mathbf{p}(i)=\mathbf{p}_{\Gamma_j}$}{
      ...
      \If{\normalsize $j\notin[n]_{>0}$}{
        {\normalsize $j\gets 1$}\;
        {\normalsize $\varphi_1,\varphi_2,\dots,\varphi_n\gets\varphi_1,\varphi_2,\dots,\varphi_n${ \otherfont shifted of }$\mathbf{d}$}\;
        {\normalsize $\mathbf{p}_{\Gamma_1},\mathbf{p}_{\Gamma_2},\dots,\mathbf{p}_{\Gamma_n}\gets\mathbf{p}_{\Gamma_1},\mathbf{p}_{\Gamma_2},\dots,\mathbf{p}_{\Gamma_n}${ \otherfont also shifted of }$\mathbf{d}$}\;

      }
    }
    {\normalsize $\mathbf{q}(\mathcal{K}\setminus\{i+N\}),c_j(\mathcal{K})\gets\argmax_{\mathbf{q}(k),c_j(k)}{l_f(\mathbf{q}(i+N),i+N)+\sum_{k\in\mathcal{K}}{l_d(\mathbf{q}(k),c_j(k),k)}}$%{ \otherfont with }$k\in\mathcal{K}=\{i,i+h,\dots,i+N\}${ \otherfont for }$\mathbf{q},\,k\in\mathcal{K}\setminus \{i+N\}${ \otherfont for }$c_j$
    }\;\vspace{-2.8ex}
    ...
    {\normalsize $\hat{\mathbf{q}}(i+h)\gets\mathbf{q}(i+h)+K(i+h)(y(i)-C\mathbf{q}(i+h))${ \otherfont returns the estimated state}}\;
    {\normalsize $\hat{y}(i+h)+C\hat{\mathbf{q}}(i+h)${ \otherfont returns the estimated energy from energy sensor }$y(i)$}\;
    ...
    {\normalsize $t_b\gets k-i$}{ \otherfont the maximum estimated battery time}\;
    {\normalsize $t_s\gets(\mathrm{diag}(\nu_j^\rho)c_j^\rho(i)+\tau_j^\rho)[\overbrace{\begin{matrix}1&1&\cdots&1\end{matrix}}^{\rho}]$}{ \otherfont time needed to do the configuration {\normalsize $c_j(i)$}}\;
    {\normalsize $t_r\gets(t_s/\overline{t})(\overline{t}-ih)$}{ \otherfont remaining time}\;
    \If{\normalsize $t_r<t_b$}{
      {\normalsize $c_j^{\rho}(i+h)\gets c_j^{\rho}$}{ \otherfont with {\normalsize $t_c\in[0,t_b]$}}\;
    }
    {\normalsize $\mathbf{p}(i+h)\gets\mathbf{p}(i)\dot{\mathbf{p}}_d(\mathbf{p}(i))/v(i)$}{ \otherfont vector field for guidance from current position and velocity {\normalfont $v(i)$}}\;
    {\normalsize $i\gets i+h$}\;
  }
  \vspace{.8ex}
