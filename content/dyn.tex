...
\fref{algo:repla}{Algorithm} derives the optimal configuration of path and computations parameters, and thus performs the coverage (re)planning and scheduling. It inputs the initial plan $\Gamma$ along with the initial time step and a guess for the energy model that we proposed in \fref{sec:period-est}{Section} and outputs the revised plan $\Gamma$ when the aerial robot reaches the final point $\mathbf{p}_{\Gamma_l}$. It optionally inputs then the initial stage that is within $[n]_{>0}$ when $\Gamma$ compromise primitive stages, $[l]_{>0}$ otherwise. In the remaining of this chapter, we discuss in detail the algorithm.
...
\begin{algorithm}[h!] %this is an example
  ...
  \ForEach{\normalsize $i\in\{t_0,t_0+h,t_0+2h,\dots\}$}{\label{algo:repla:foreach}
    ...
    {\normalsize $P(i+h)^-\gets A_d P(i)A_d'+S$}\;
    {\normalsize $K(i+h)\gets (I+K(i+h)C)P(i+h)^-$}\;
    {\normalsize $\hat{\mathbf{q}}(i+h)\gets${ \otherfont compute }$\mathbf{q}(i+h)+K(i+h)(y(i)-C\mathbf{q}(i+h))${ \otherfont from energy sensor }$y(i)$}\;
    ...
    \While{\normalsize $b_d(k)>0$}{
      ...
      {\normalsize $b_d(k+h)\gets b_d(k)-k_b\left(V-\sqrt{V^2-4R_rC\mathbf{q}(k+h)}\right)/(2R_rQ_c)$}\;
      {\normalsize $k\gets k+h$}\;\label{algo:repla:battevo}
    }
    {\normalsize $t_b\gets k-i$}\;%the maximum estimated battery time
    {\normalsize $t_s\gets(\mathrm{diag}(\nu_j^\rho)c_j^\rho(i)+\tau_j^\rho)[\overbrace{\begin{matrix}1&1&\cdots&1\end{matrix}}^{\rho}]$}\;%time needed to do the configuration $c_j(i)$
    {\normalsize $t_r\gets(t_s/\overline{t})(\overline{t}-ih)$}\;%remaining time
    \If{\normalsize $t_r<t_b$}{
      {\normalsize $c_j^{\rho}(i)\gets${ \otherfont find }$c_j^{\rho}${ \otherfont with }$t_c\in[0,t_b]${\otherfont , otherwise take }$\underline{c}_j^\rho$}\;
    }
    \vspace{.8ex}
...
The algorithm iterates at a each time step $h\in\mathbb{R}_{>0}$ on \fref{algo:repla:foreach}{Line}. The size of the step is related to the accuracy and to the time needed to replan $\Gamma$. Nonetheless, there are further constraints on $h$ due to the numerical simulation algorithm: $h$ has to be small enough ($h\rightarrow 0$) so that the numerical simulation (at \fref{algo:repla:mpc}{Line}, \fref{algo:repla:sysevo}{}, \fref{algo:repla:sysevobatt}{}, and \fref{algo:repla:battevo}{}) does not diverge. To this end, a typical value for $h$ that we adopted is 1/100 of a second; there are various techniques to estimate $h$ more accurately, e.g., by running the numerical simulation and analyzing th error of two different guesses of $h$~\citep{iserles2009first}; however, these are beyond the scopes of this work. With this $h$, Euler method\findex{Euler method} that we use in \frefeqM{eq:mpc-cost-euler}{eq:batt-euler} converges to the real solution--see~\citep{iserles2009first,atkinson2009euler} for a formal proof--whereas for problems with higher accuracy we advise the Runge-Kutta methods in \fref{sec:rk4}{Section} that are among the most popular methods for numerical simulation~\citep{atkinson2009euler}. These methods use quadrature\findex{quadrature}, a procedure that replaces the integral with with a finite sum~\citep{iserles2009first}. We use the Runge-Kutta inside \powprof{} in \fref{sec:powprof}{Section}, whereas propose the Euler method in the algorithm for its relative simplicity.

\fref{algo:repla:endcheck}{Lines}\fref{algo:repla:return}{--\hspace*{-.8ex}} verify whenever the algorithm reached the final point $\mathbf{p}_{\Gamma_l}$ in \fref{def:trigs}{Definition} and return the replanned plan $\Gamma$ in this latter eventuality. On \fref{algo:repla:istrig}{Lines}\fref{algo:repla:shifttrigs}{--\hspace*{-.8ex}} then the algorithm switches stages when the aerial robot reaches a triggering points (also in \fref{def:trigs}{Definition}). If $\Gamma$ contains $n$ primitive paths rather than all the stages up to $l$ explicitly, it updates the paths and the triggering points with the shift $\mathbf{d}$ on \fref{algo:repla:shiftpaths}{Lines}\fref{algo:repla:shifttrigs}{--\hspace*{-.8ex}}. It then selects the energy aware configuration of computations on \fref{algo:repla:mpc}{Line}, by solving the NLP in \frefeq{eq:disc-ocp-output-mpc} (derived from the OCP \frefeq{eq:ocp-output-mpc} using the multiple shooting method in \fref{sec:multi-shoot}{Section}). It thus obtain the trajectory of the controls and states for a given horizon $N\in\mathbb{R}_{>0}$ expressed in seconds. In particular, the control trajectory contains $|\mathcal{K}|-1$ items, whereas the state trajectory $|\mathcal{K}|$ items. This discrepancy in the number of sets of the two trajectory is due to the construction of OCP: we provide an initial guess for the state $\mathbf{q}(i)$ at time instant $i$, whereas derive the control $c_j(i)$ from the solution to the OCP. For the horizon $N$, we adopted the value of 10 seconds, meaning the algorithm evolves the model in \frefeq{eq:perf-model-in-mpc} (coming from \frefeq{eq:state-perf}) for $N$ future seconds and selects the control trajectory $c_j^*$ that minimizes the costs in \frefeq{eq:mpc-cost-euler} and in \frefeq{eq:final-cost-mpc}. The higher the horizon, the better the accuracy. A typical value in the aerial robotics literature that implements MPC~\citep{gavilan2015iterative,kang2009linear,stastny2018nonlinear,chao2011collision} is in the dozens of seconds, e.g., it is fourteen in Gavilan~et~al., ten and fourty in Kang~and~Hedrick, from two to eight in Stastny~and~Siegwart, and five in Chao~et~al.

In order to solve the NLP in \frefeq{eq:disc-ocp-output-mpc}, the algorithm evaluates the constraints \frefeqM{eq:disc-dyn-evol}{eq:disc-polyg-const} ($t_0$ in the latest two constraints is equal to $i$), where it has to numerically simulate both the energy and the battery models from $i$ to $i+N$ with a step size of $h$. A possible optimization that considerably speeds up the derivation of the optimal control is to use different horizons for numerical simulation and for the constraints, i.e., $\mathcal{H}={i,i+nh,i+2h,i+N}$, where $n\in[1,1/h]$. For instance, when $n$ is equal to $1/h$, the algorithms adds the constraints at each second, but numerically simulates the models on $\mathcal{K}$. Another optimization is to move the constraint in \frefeq{eq:disc-polyg-const} out of the NLP and check whenever the aerial robot is flying the obstacle and thus require $c_j^\rho={0}$. 

On \fref{algo:repla:sysevo}{Lines}\fref{algo:repla:endkalmy}{--\hspace*{-.8ex}} the algorithm estimates the periodic model's in \fref{sec:periodic-model}{Section} using Kalman filter in \fref{sec:kalmy}{Section} from energy sensor's measurements $y(i)$. The Kalman filter\findex{Kalman filter} has several interesting properties over other methods for state estimation, and among the others minimizes the variance of the estimation mean square error\findex{mean square error} (MSE)~\citep{kalman1960new,simon2006optimal,jwo2007practical}.
