...
\end{equation}
their gradient then points away from the base of the plane in \frefeq{eq:pathf-line} and the center of the circle in \frefeq{eq:pathf-circle} in \fref{fig:grad}{Figure}. If the goal is not to fly over the circle in \fref{fig:plot11}{Figure}, but to its center $(x_c,y_c)$ in \frefeq{eq:pathf-circle}, the vector field $\varPhi$ in \frefeq{eq:vec-field-def} direct us in the opposite direction; we can then use $-\nabla\varphi_i$ to direct the robot to the goal.
