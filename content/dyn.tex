...
To define the cellular decomposition algorithms that decomposes the space to be covered into sub-regions, we define robot's free space\findex{free space}; where the robot is free to move as $\mathcal{W}$ for an inertial navigation frame $\mathcal{O}_W$. $\mathcal{W}\subset\mathbb{R}^2$ contains all the points delimited by the vertices of a polygon $v$ except for $i$ obstacles delimited by the vertices of $i$ polygons $o_i$.

The intuition behind the algorithm is to sweep $W$ with a straight line and add a subregions when it encounters critical points~\citep{choset2001coverage,choset2005principles,choset2000coverage}. Since we know the vertices and the polygon, it is enough to verify the critical points by iterating the vertices, and visit the constructed adjacency graph to find the coverage order. This algorithm is in the robotics research literature under the name of boustrophedon decomposition\findex{boustrophedon decomposition}~\citep{choset2000coverage} and it optimizes the neighboring sub-regions that can be aggregated compared to trapezoidal decomposition~\citep{galceran2013survey}--another algorithm that simple add a sub-region when it encounters a vertex opposed to critical points or change in connectivity~\citep{lavalle2006planning}. The intuition of using critical points\findex{critical points} has been widely studied in the literature in the context of roadmaps~\citep{canny1988complexity,canny1988constructing,canny1993opportunistic}\findex{roadmaps} and coverage~\citep{choset2000exact}.
