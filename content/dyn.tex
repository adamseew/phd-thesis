...
Once we delimited the coverage area and obstacles areas into appropriate cells, we need to define the path that covers the coverage area. A classical approach in the literature is to sweep the polygon enclosed by the vertices $v$ while avoiding the obstacles using back and forth motion. We saw in \fref{cp:soa}{Chapter} and discussed briefly in the previous section that this is often termed boustrophedon motion. We propose an analogous version of the boustrophedon motion. We recall briefly the intuitive path in \fref{sec:path-wise}{Section} in \fref{fig:plot3}{Figure}: there we proposed what we call boustrophedon like-motion. We have proposed this latter variation as an optimization for the aerial robot flying that to increase the efficiency during turns already broadly discussed in the coverage literature for both mobile~\citep{huang2001optimal} and aerial robots~\citep{artemenko2016energy,li2011coverage}. Indeed the original boustrophedon motion has edges parallel to the polygon where the mobile robot has to slow whereas an aerial robot has to rely on external turning structures such as curlicue orbits~\citep{xu2011optimal,xu2014efficient}. A way of covering the area in \fref{fig:bcd4}{Figure} is in \fref{}{Figure} that shows the boustrophedon-like motion for cell $c_1$.
% add figure
The remaining cells are covered in the same manner and the overall coverage is obtained by visiting the cells in the appropriate order derived in the previous section ($c_1,c_2,c_4,$ and $c_3$). Let us assume for an instant that the turning radius in \fref{pb:cov-pb}{Problem} is not given. Let us further assume that the aerial robot can overfly the boundaries of the polygons $v$ and $o_i$ for the turns. The methodology is to lay the line $\varphi_1$ from \fref{eq:plot3}{Figure} parallel to the edge formed by vertices $v_1$ and $v_4$ and the center of the circle $\varphi_2$ laying on the edge formed by vertices $v_4$ and $v_3$. 
