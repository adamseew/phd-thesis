...
There are many autonomous use cases involving aerial robots, such as precision agriculture, search and rescue, payload delivery, transportation, and many others. To formulate a mobile robot planning problem later in \fref{sec:pb-form}{Chapter}, we focus on precision agriculture and progressively build dynamic planning of a fixed-wing aerial robot flying over an agricultural field with little human input.
...
In the remainder of the chapter and before we introduce the problem in \fref{sec:pb-form}{Chapter}, we briefly investigate the evolution of the field of aerial robotics in the next section. We then analyze the main aerial robots available today, and how they apply to dynamic energy planning in \fref{sec:aerial-robo-types}{Section}. We then provide some further motivation for our planning in \fref{sec:motivation}{Section} and the outline of the approach in \fref{sec:outline}{Section}. Finally, we provide the structure of the remaining chapters in \fref{sec:structure}{Section}.

This chapter connects to the remainder of this work as follows. Here we introduce and motivate the dynamic energy planning for autonomous aerial robots. We formalize the planning problem in \fref{sec:pb-form}{Chapter}. We describe the derivation of a proper energy model to predict the future energy consumption of the planning problem in \fref{cp:model}{Chapter}. We estimate some coefficients of the model using robust estimation techniques in \fref{cp:est}{Chapter}. We implement an optimal configuration of the path and computations using data-driven control\findex{data-driven control} and other modern optimal control techniques in \fref{cp:opt}{Chapter}. We use such configuration for guidance and scheduling of the aerial robot in \fref{cp:gd}{Chapter}. The guidance moves physically the robot; the scheduling defines the granularity of the tasks being executed in flight. Moreover, we discuss previous approaches to solve the dynamic planning problem in \fref{cp:soa}{Chapter}. 
...
\begin{figure}[!h]
  ...
  \begin{tikzpicture}[auto, node distance=3.5cm,>=latex']
      \node [block,node distance=1cm] (theblock) {Planning Problem};
      \node [right of=theblock] (arrow) {\fref{cp:intro}{Chapter}};
      \node [block,below=1cm] (theblock2) {Optimal Control Generation};
      \node [right of=theblock2,node distance=3.54cm] (arrow2) {\fref{cp:opt}{Chapter}};
      \node [block,below left=.5cm and -.5cm of theblock2] (theblock3) {Energy Models};
      \node [right of=theblock3,node distance=2.1cm] (arrow3) {\fref{cp:model}{Chapter}};
      \node [block,below right=.5cm and -.5cm of theblock2] (theblock4) {Guidance};
      \node [right of=theblock4,node distance=1.8cm] (arrow4) {\fref{cp:gd}{Chapter}};
      \node [block,below=.5cm and -.5cm of theblock3] (theblock5) {State Estimation};
      \node [right of=theblock5,node distance=2.14cm] (arrow5) {\fref{cp:est}{Chapter}};
      \draw [->] (arrow) -- node{} (theblock);
      \draw [->] (theblock) -- node{} (theblock2);
      \draw [->] (arrow2) -- node{} (theblock2);
      \draw [->] (theblock3) -- node{} (theblock2);
      \draw [->] (arrow3) -- node{} (theblock3);
      \draw [->] (theblock2) -- node{} (theblock4);
      \draw [->] (arrow4) -- node{} (theblock4);
      \draw [->] (theblock5) -- node{} (theblock3);
      \draw [->] (arrow5) -- node{} (theblock5);

      \draw [decorate,decoration={brace,amplitude=10pt,mirror,raise=35pt},yshift=0pt]
      (theblock5.north west) -- (theblock5.north west -| theblock4.north east) node [black,midway,yshift=-18ex] (notextnode) {$\Gamma$};
      \node [block,below=.2cm and -.5cm of notextnode] (theblock6) {Implementation};
      \node [right of=theblock6,node distance=2.14cm] (arrow6) {\fref{cp:imp}{Appendix}};

      \draw [->] (notextnode) -- node{} (theblock6);
...
In this chapter, we introduced the dynamic energy planning for aerial robots. We define the planning problem in \fref{sec:plan-pb}{Chapter}, and propose the solution to the problem in \fref{cp:opt}{Chapter}, which builds upon the topics presented in the remaining chapters. In particular, in \fref{cp:soa}{Chapter} we present the state-of-the-art in energy modeling and planning for mobile robots. We include some energy models for heterogeneous computing hardware.
