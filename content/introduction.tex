...
In summary, aerial robots have a relatively recent past. Some initial experiments of unmanned fligt were performed shortly after the first heavier-than-air manned powered flight. Modern aerial robots have been applied to many autonomous scenarios and their is expected to increase. Aerial robots are expected to grow significantly in many areas of robotics research raging from agriculture to planetary exploration. For the latter, \fref{fig:ingenuity}{Figure} shows NASA's Ingenuity Mars Helicopter. A small coaxial aerial robot that performed the first powered, controlled flight on another planet on April 19, 2021. Aerial robots for planetary exploration are expected to be further deployed in future explorations of the Saturn's moon Titan~\citep{voosen2019nasa}.
...
A further complication in these scenarios is that autonomous mobile robots usually carry energy-demanding heterogeneous computing hardware\findex{heterogeneous computing hardware} along a microcontroller\findex{microcontroller}. Computing hardware is involved in the planning and enables autonomous functions by running computer vision and other complex algorithms, opposed to the microcontroller. The latter runs motion primitives by directly interfacing actuators (such as servos\findex{servo} for the control surfaces\findex{control surface}) and motors\findex{motor}.
...
As an example, we refer to an aerial robot in a precision agriculture scenario (see \fref{fig:opterra}{Figure}), that covers an agricultural field. In abstract terms, the field is represented by a polygon (see \fref{fig:plot2}{Figure}) and the aerial robot detects some patterns on such polygon using convolutional neural network (CNN). The aerial robot further communicates the position of the pattern to other, ground based actors. Detecting the patterns usually involves heterogeneous computing elements (GPU\findex{GPU} and/or multiple CPU\findex{CPU} cores) and consumes a significant amount of energy opposed to the communication. Detection is a computation, while the communication is a (energy-inexpensive) task.
...
The approach that we present in this work combines, generalizes, and extends some past body of knowledge (see \fref{cp:soa}{Chapter}) on mobile robot planning problem described formally in \fref{sec:pb-form}{Section}. The approach addresses the increasing computational demands and their relation to energy consumption, path, and autonomy for the mobile robot planning problem. We apply the approach to aerial robots in different autonomous scenario. 
...
We use this concept to model complex paths. For instance, the path might contain multiple circles and lines, as shown in \fref{sec:flight-plan}{Subsection}. The aerial robot travels the path using a vector field for guidance~\citep{de2017guidance} that converges to the path. We describe in detail such vector field in \fref{cp:gd}{Chapter}. The robot switches between the trajectories as soon as it reaches specific triggering points. 
...
\begin{figure}[p]
  ...
  \caption[Energy data of a fixed-wing aerial robot]{Empirical energy data collected flying a fixed-wing aerial robot plan and cover the polygon in \fref{fig:plot2}{Figure}. The data shows how the energy signal is periodic over time with the fixed-wing aerial robot reiterating a set of paths.}
  \label{fig:energy-1}
\end{figure}
\begin{figure}[p]
  \centering
  ...
  \caption[Power spectrum of the energy data of a fixed-wing aerial robot]{Power spectrum derived from the empirical energy data of a fixed-wing aerial robot in \fref{fig:energy-1}{Figure}. The spectrum justifies further the choice of a periodic energy model to predict future energy consumption of the aerial robot.}
  \label{fig:spectrum-1}
\end{figure}

Some collected energy data from the Opterra (in \fref{fig:opterra}{Figure}) adapted for precision agriculture scenario along its power spectrum back the choice of the periodic energy model. The energy needed to fly a fixed-wing aerial robot plan and cover the polygon in \fref{fig:plot2}{Figure} (we describe the fixed-wing aerial robot plan in detail in \fref{sec:flight-plan}{Section}) is shown in \fref{fig:energy-1}{Figure}. \fref{fig:spectrum-1}{Figure} shows its frequency spectrum (the data are uniformly sampled).  The spectrum is centered at 0 frequency. The frequency 0 peaks at 4$\cdot$10${}^{5}$ and indicates the shift from on the power-axis in \fref{fig:energy-1}{Figure}. The energy data are thus periodically repeated (with noise) using three frequencies. To obtain the spectrum in frequency space, we computed the Fourier transform.
...
Before formulating the mobile robot planning problem in \fref{sec:plan-pb}{Section}, we provide a formal definition of some of the main constructs that we use in the remainder of this work in \fref{sec:definitions}{Section}, along the derivation of the example of a precision agriculture scenario that we extend in each chapter in \fref{sec:flight-plan}{Section}.
...
\begin{highlight}
  \begin{defn}[Computations and motion]\label{def:comps}
...
We note that the altitude $h$ from \fref{def:stage}{Definition} is not strictly fixed. It might change at different flying phases for an aerial robot, and under different atmospheric conditions.
...
We then define the plan in \fref{def:plan}{Definition} as a finite state machine. It returns at any point in space a specific stage.
...
The plan that we show in \fref{fig:state-machine}{Figure} does not have backwards loops. We discuss other more complex plans in \fref{cp:conc}{Chapter}. 
...
\begin{figure}[p!]
  ...
  \caption[Fixed-wing aerial robot's plan to cover a regular polygon with four sides]{Fixed-wing aerial robot plan to cover a regular polygon with four sides. The plan covers the polygon with the same principle of the intuitive plan in \fref{fig:plot3}{Figure} but preserving large turns necessary for the flight of a fixed-wing craft. Likewise in \fref{fig:plot3}{Figure}, the entire polygon is covered iterating the primitive paths $\varphi_1,\varphi_2,\varphi_3,\varphi_4$ (gray lines). The dashed blue line indicates the triggering points, and the black line is the planned flight. The height and length of the polygon are $y_{\Gamma_3}-y_{\Gamma_1}$ and $lx_d/4$.}
  \label{fig:plot4}
\end{figure}
...
\end{align}
\end{subequations}
where index $i$ is defined the same way as in \frefeq{eq:line-gene} along the shift and integer division. $r_1$ and $r_2$ are given radiuses of the circles (see \fref{fig:plot4}{Figure}).
...
\begin{figure}[p!]
  ...
  \caption[Alteration of a path parameter of the fixed-wing aerial robot's plan]{Alteration of a path parameter of the fixed-wing aerial robot's plan in \fref{fig:plot4}{Figure}. The black line is the un-altered path, the dashed blue line the replanned path. The gray area delimits the bound $\mathcal{C}_{4,1}$. The alteration changes the distance between the survey lines hence extending or shortening the flying time. The figures show the concept for the path parameter $c_{4,1}$. Same principle applies to path parameter $c_{8,1}$ for the next two survey lines, $c_{12,1}$, and so on.}
  \label{fig:plot5}
\end{figure}
...
\begin{pb}[Mobile robot planning problem]\label{pb}
  Consider an initial plan $\Gamma$ from \fref{def:stage}{Definition}. We are interested in the planning of the path and computation parameters $c_i,\,\forall i\in[l]^+$ under energy constraints.
...
\fref{cp:soa}{Chapter} presents the state of the art in energy modeling and planning for mobile robots and includes the available energy models for heterogeneous computing hardware used on mobile robots.

\fref{cp:model}{Chapter} elaborates on energy models for computations, motion, and the battery. It derives and formally proofs a periodic energy model that we use in the remainder of this work to predict the future energy consumption while solving the mobile robot planning problem.

\fref{cp:est}{Chapter} describes the ways which we use to estimate the coefficients of the energy models and hence refine the future predictions with the data.

\fref{cp:gd}{Chapter} propose an approach to guide the aerial robot on the path derived from the approach.

\fref{cp:opt}{Chapter} presents the optimization algorithm to select the energy optimal parameters configuration for the path and computations simultaneously.

\fref{cp:conc}{Chapter} concludes our work and presents some recommendations for future research.

We present some additional information in the appendices.

\fref{app:imp}{Appendix} propose the implementations of the planning problem, the model, the estimation technique, the guidance action, and the derivation of the optimal control.  
