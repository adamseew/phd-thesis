...
In the search and rescue scenario, the aerial robot performs autonomous detections of patterns over the sea. In this scenario, we mostly focus on the validation of the computation energy model (described in detail in \fref{sec:comp-ener-model}{Section}).
...
Before formulating the mobile robot planning problem in \fref{sec:plan-pb}{Subsection}, we first provide the formal definition of some of the main constructs we will use in the remainder of this work.
...
Parameters are variable values than might be either reals $\mathbb{R}$ or integers $\mathbb{Z}$. We use such values to dynamically plan path and computations. $c_{i,j}$ denotes the $j$-th parameter of the $i$-th parameters set $c_i$
\begin{equation}
...
The altitude $h$ from \fref{def:stage}{Definition} might change at different flying phases and under different atmospheric conditions
...
To move from a given stage $i$ to the next stage $i+1$, we define specific points. As soon as the mobile robot reaches the proximity if these points in switches to the next stage.
...
We show the definition of plan, stage, trigerring, and final point visually with \fref{fig:state-machine}{Figure} and \fref{fig:state-machine2}{Figure}.
...
\begin{pb}[Mobile robot planning problem]\label{pb}
  ...
  We are further interested in the guidance to the path, and scheduling of the computations resulting from such planning.
\end{pb}    
\end{highlight}
