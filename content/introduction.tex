...
For instance, detecting ground patterns usually involves heterogeneous computing elements (GPUs\findex{GPU} and/or multiple CPU\findex{CPU} cores) and consumes a significant amount of energy opposed to the log of the position. Detection is a computation, while a log script is (an energy-inexpensive) task.
...
The concept is shown in \fref{fig:plot1}{Figure} for $c$ zero and $a,b$ two and minus one respectively. A generic point $\mathbf{p}:=(x_{\mathbf{p}},y_{\mathbf{p}})$ intersects the plane 
\begin{equation}
  \varphi(x,y):=2y-x,
\end{equation}
