...
%\lettrine{E}{nergy is a central aspect} of countless mobile robotics scenario and oftentimes a limiting factor to further performance as we discussed in the previous chapters. To optimize the energy efficiency of the mobile robots we, however, first need an accurate energy model that predicts future energy consumption. In this chapter we deal with this latter problem: the derivation of an energy model for all the parts of an aerial robot. In this work, we particularly emphasize the computations energy against the motional energy, and later propose an approach to deal with these aspects altogether while solving the \fref{pb}{Problems}\fref{pb:cov-pb}{--\href{-.8ex}}, the problems of providing a coverage for a given space and to replan it energy-wise while simultaneously providing a power saving schedule. To this end, our energy model needs to compromise both these aspects; the computations and the motion. We saw in \fref{cp:model}{Chapter} some computational energy models and discussed some energy-aware approaches of aerial robots (and mobile robots broadly) performing coverage path planning (CPP), or motion planning generally.
