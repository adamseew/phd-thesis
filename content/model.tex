...
\subsection{\color{red}Fourier series of empirical data}
\label{sec:fourier}


\section{\color{red}Periodic Energy Model}

Let us suppose the aerial robot is operating in an autonomous scenario. Such a robot is often supposed to iterate a set of tasks and paths (see \fref{sec:outline}{Section}) in a periodic fashion. For instance, the aerial robot might be covering a polygon in \fref{sec:flight-plan}{Section}. Since the paths and tasks are periodically iterated over time, we expect the energy to evolve similarly. We will see the assumption of the periodic energy model being eased in practice to quasi aperiodic or aperiodic evolutions in \fref{sec:non-perio}{Section}. 

We motivate the choice of a periodic energy model further with some empirical energy data of the Opterra fixed-wing aerial robot flying the agricultural scenario (see \fref{sec:flight-plan}{Section}) in \fref{fig:energy-1}{Figure} and \fref{fig:spectrum-1}{Figure}.

In the remainder of this section, we first derive a differential periodic energy model in \fref{sec:periodic-model}{Section}. We enhance the model with the path and computations parameters in \fref{sec:nom-cont}{Section}, allowing us to predict the energy consumption of a given configuration of parameters. We explain the means by which we convert the parameters into actual energy consumption in \fref{sec:merging}{Section}, and discuss the model's behavior for aperiodic energy evolutions in \fref{sec:non-perio}{Section}.

\subsection{Derivation of the differential periodic model}
\label{sec:periodic-model}

We refer to the instantaneous energy consumption evolution simply as the energy signal. We model the energy using energy coefficients $\mathbf{q}\in\mathbb{R}^m$ that characterize such energy signal. The coefficients are derived from Fourier analysis (the size of the energy coefficients vector $m$ is related to the order of the Fourier series in \fref{sec:fourier}{Subsection}) and estimated using a state estimator (see \fref{cp:est}{Chapter}). We prove a relation between the energy signal and the energy coefficients in \fref{lem:eqv}{Lemma}. We show after the main results how this approach allows us variability in terms of non-periodic signals. After illustrating the energy model, we enhance it with the energy contribution of the path in and of the computations in \fref{sec:merging}{Subsection}. 
...
\begin{proof}
We propose a formal proof of the lemma. The proof justifies the choice of the items of the matrices $A,C$ and of the initial guess $\mathbf{q}_0$ in \frefeqM{eq:state-details}{eq:mat_C}. We write these elements such that the coefficients of the series $a_0,\dots,b_r$ are the same as the coefficients of the state $\alpha_0,\dots,\beta_r$.
...
The solution at time $t$ of the model in \frefeq{eq:state-perf} can be expressed
\begin{equation}
...
Thereby we first prove that the first terms of the \frefeq{eq:proof-complex} and \frefeq{eq:proof-output} match. We find the eigenvalue from $\det(0)=0$, which is $\lambda_0=0$. The corresponding eigenvector can be chosen arbitrarily
\begin{equation}
...
Then, we prove that all the terms in the sum of both the \frefeq{eq:proof-complex} and \frefeq{eq:proof-output} match. 
...
\begin{equation}\label{eq:state-control2}
  \mathbf{c}_i(t)=\begin{bmatrix}c_i^\rho(t) & c_i^\sigma(t)\end{bmatrix}',
\end{equation}
...
\begin{highlight}
  \begin{obs}
    We observe that:
    \begin{itemize}
      \item A change in path parameters affects the energy indirectly. It alters the time when the aerial robot reaches the final point $\mathbf{p}_{\Gamma_l}$.
...
We use this observation later in \fref{sec:opt-cont-gener}{Subsection} to check that the battery discharge time is greater and replan the path parameters accordingly.  We replan the computation parameters to maximize the instantaneous energy consumption against the maximum battery discharge rate.
...
\end{align}\end{subequations}
scaling factors quantifying the contribution to the plan of a given parameter in terms of time for the first $\rho$ parameters, and power for the remaining $\sigma$ (we use the same notation for the path and computation scaling factors as for the parameters). This is derived directly from the observation.
...
\subsection{Parameters domain transformation}
\label{sec:merging}
...
The path parameters allow changing the path. They are a way to alter the nominal trajectory in the initial plan and thus alter the energy by changing the flying time in \fref{fig:tee1}{Figure}.
In fact, the algorithm uses the set from \frefeq{eq:area} to find the path parameters such that the plan consisting of flying $\varphi_i$ has the highest energy, while still respecting the constraints. In \fref{fig:tee2}{Figure}, the parameter radius of the circle $c_{1,1}$ is replanned as, e.g., averse atmospheric conditions (headwind in \fref{fig:tee3}{Figure}) do not allow to terminate the plan.
...
The scaling factors for the path parameters from \frefeq{eq:scaling} are derived empirically. For example, we can obtain the scaling factor $\nu_{1,1}$ in \fref{fig:tee1}{Figure} measuring the time needed to compute the path with the lowest configuration $\underline{c}_{1,1}$, $\underline{t}$ and the highest $\overline{t}$. 
...
Let us recall from \fref{def:stage}{Definition} that the $i$-th stage $\Gamma_i$ of the plan $\Gamma$ contains the computation parameters which characterize the computations. We estimate the energy cost of these computations using {\small\tt{powprofiler}}, the open-source modeling tool from \fref{sec:powprof}{Subsection}.
...
\subsection{Aperiodic energy evolution}
\label{sec:non-perio}
