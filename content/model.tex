...
\begin{figure}[p]
  ...
  \caption[Trajectory of the aerial robot flying the highest path configuration]{The trajectory of the aerial robot flying paths $\varphi_1,\varphi_2,\dots,\varphi_l$ from \fref{sec:flight-plan}{Section} with the highest configuration of the path parameter $c_{i,1}=\overline{c}_{i,1}$. This configuration of path parameter has the longest time but the shortest distance between the lines and thus the highest quality of the coverage in \fref{fig:plot2}{Figure}.}
  \label{fig:plot-8}
\end{figure}
\begin{figure}[p]
  \centering
...
The first bullet point in the observation can be verified by inspection of the example in \fref{sec:flight-plan}{Section}. It is clear that if we increase the parameter $c_{i,1}$, which is relative to the radius of the circle, the flying time decreses. This is further shown in \fref{fig:plot-8}{Figure} and \fref{fig:plot-8}{Figure}. \fref{fig:plot-8}{Figure} illustrates the trajectory of the aerial robot (composed of all the paths $\varphi_1,\varphi_2\,\dots,\varphi_l$) flying at the highest configuration of the path parameter $c_{i,1}=\overline{c}_{i,1}$. \fref{fig:plot-9}{Figure} then illustrate the trajectory flying at the lowest configuration $c_{i,1}=\underline{c}_{i,1}$. The flying time differs significantly, along with the quality of the coverage of the polygon (the agricultural field) in \fref{fig:plot2}{Figure}.

In \fref{fig:plot-9}{Figure}, the parameter $c_{1,1}$ that alters the radius and center of the upper circle (defined originally in \fref{sec:flight-plan}{Section}) is replanned as, e.g., averse atmospheric conditions do not allow to terminate the original plan in \fref{fig:plot-8}{Figure}.
...
$\nu_{i,j}\in\nu_i,\,\forall j\in[n]_{>0}$ where $n=\rho+\sigma$.
...
The scaling factors for the path parameters from \frefeq{eq:scaling} are derived empirically. For example, we can obtain the scaling factor $\nu_{1,1}$ relative to the alteration of the upper circle $c_{i,1}$ from \fref{sec:flight-plan} measuring the time needed to compute the path with the lowest configuration $\underline{c}_{1,1}$, $\underline{t}$ in \fref{fig:plot-9}{Figure}, and the highest $\overline{t}$ in \fref{fig:plot-8}{Figure}. 
...
The concept of a path and a computation parameter scale transformation is illustrated in \fref{fig:plot-6}{Figure}. The control constraint set $\mathcal{U}_i$ extends to both the flight time and energy domain. The energy domain is bounded by the output of the \powprof{} tool, while the flight time domain by the empirical data. The dynamic energy planning selects the highest possible configuration under the constraints. Currently, the highest configuration of parameters (the optimal control law $c^*_i(t)$) corresponds to the highest configuration of parameters $(\overline{c}_{i,1},\overline{c}_{i,2})$ (blue point in the figure). We will show in \fref{sec:output-mpc}{Section} the optimal control derivation over a time horizon $N$ under given battery constraints. 
