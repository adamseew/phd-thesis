...
We assume the mobile robot carries a battery energy sensor and obtain the initial state of charge $b(y(t_0),t_0)$ utilizing the output of such sensor. This is often the case of aerial robots with a flight controller, which returns various matrix including the battery state of charge. 
...
The dynamic evolution in \fref{eq:dyn-evol}{Equation} is then the periodic model in \frefeq{eq:state-perf} together with the scale transformation from \fref{sec:merging}{Section}
\begin{equation}\label{eq:perf-model-in-mpc}
  f(\mathbf{q}(t),c_i(t),t)=A\mathbf{q}(t)+B\mathrm{diag}(\nu_i)(c_i(t)-c_i(t-1)),
\end{equation}
where $c_i(t-1)$ is the control at the time instant preceding $t$.
...
\end{equation}
where $l$ is given in \fref{eq:insta-cost-mpc}{Equation}.

The discrete dynamic evolution in \fref{eq:disc-dyn-evol}{Equation}
\begin{equation}
  f_d(\mathbf{q}(k),c_i(k),k)=A_d\mathbf{q}(k)+B\mathrm{diag}(\nu_i)(c_i(k)-c_i(k-h)),
\end{equation}
where $A_d$ is the discretized version of the state transition matrix $A$ and for small enough interval of $h$ is 
\begin{equation}
...
We transformed the OCP in \frefeq{eq:} into an NLP in \frefeq{} by first discretizing and thus effectively implementing the direct multiple shooting method in \fref{sec:multi-shoot}{Section}. We note that we can implement the single shooting method by keeping only the initial state as the decision variable $\hat{\mathbf{q}}_0$ opposed to 
using the interval boundary time points as decision variable in the multiple shooting method~\citep{rawlings2017model}.
