...
\begin{figure}[t!]
  ...
  \caption[Concept of a line as a path function]{The path is mathematical function $\varphi(\mathbf{p}(t))=h$ that represents a line at an altitude $h$. A generic point $\mathbf{p}$ in 2D space intersects the plane formed by $\varphi$ at a given value of the $z$-axis $d$.}
  \label{fig:plot1}
\end{figure}
...
\begin{figure}[t!]
  ...
  \caption[Concept of a circle as a path function]{The path now represents a circle at an altitude $h$. $\mathbf{p}$ intersects the cone formed by $\varphi$ at a given value of the $z$-axis $d_2$.}
  \label{fig:plot11}
\end{figure}
...
\begin{equation}\label{eq:basic-path}
  \varphi(x,y):=ax+by+c,
\end{equation}
where $a,b,c\in\mathbb{R}$ are given constants. The generic point $\mathbf{p}(t)$ intersects $\varphi(x,y)$ on a specific value of the $z$-axis. We illustrate the concept in \fref{fig:plot1}{Figure} for $c$ zero and $a,b$ minus one and two. The point intersects the plane formed by the path function
\begin{equation}
  \varphi(x,y):=2y-x,
\end{equation}
...
For simplicity, we merge the computations and path constraint sets in a single constraint set. $i$th stage constraint set 
...
\end{figure}
In \fref{fig:state-machine-loop}{Figure} we illustrate a plan composed of $n$ primitive stages $\Gamma_1,\Gamma_2,\dots,\Gamma_n$ (containing primitive paths $\varphi_1,\varphi_2,\dots,\varphi_n$) that are reiterated with the shift $\mathbf{d}$. $t_3<t_4$ is a time instant likewise in \fref{fig:state-machine}{Figure}.
...
There are thus two computations parameters. A computation parameter is the FPS rate, and another computation parameter the encryption binary value. The CNN ROS node computation parameter is $c_{i,2}$, and the encryption ROS node computation parameter is $c_{i,3}$.
