...
\newcommand{\edition}{First}
\newcommand{\editionyear}{2021}
\newcommand{\editionmonth}{October}
...
\input{frontmatter/glossaries_entries}
\makeglossaries
...
\cleardoublepage
\pagestyle{fancy}
\input{frontmatter/abstract}\thispagestyle{empty} 
\input{frontmatter/ack}\thispagestyle{empty} 
\input{frontmatter/publications}\thispagestyle{empty} 
\pagestyle{fancy}{\setttsize{\small}\tableofcontents}\cleardoublepage\thispagestyle{empty} % toc
...
\setttsize{\small}
\pagestyle{fancy}\printbibliography[env=biblioseight,title=References,notkeyword=j,notkeyword=cn,notkeyword=ws,notkeyword=sw,notkeyword=p]
